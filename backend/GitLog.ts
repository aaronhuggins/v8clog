...
export class GitLog {
  ...
  async clone (config: [key: string, value: string][] = []) {
    ...
    for (const [key, value] of config) configs.push('--config', `${key}=${value}`)

    const cmd = Deno.run({
      cmd: ['git', 'clone', '--bare', '--filter=blob:none', ...configs, this.#repo],
      stdout: "piped",
      stderr: "piped"
    })
  ...
  async log ({ author, revision, files = [] }: LogOptions = {}) {
    ...
    const [status, yaml = new Uint8Array(0), stderr = new Uint8Array(0)] = await Promise.all([
      cmd.status(),
      cmd.output(),
      cmd.stderrOutput()
    ])
